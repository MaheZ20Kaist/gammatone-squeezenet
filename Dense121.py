...
'''def encoder(input_shape = (INPUT_SIZE[0],INPUT_SIZE[1],1)):
    ...
    y = tf.keras.layers.Conv2D(kernel_size=3, filters=16, padding='same', use_bias=True, activation='relu')(x)
    y = tf.keras.layers.MaxPooling2D(pool_size=2)(y)
    y = tf.keras.layers.Conv2D(kernel_size=3, filters=16, padding='same', use_bias=True, activation='relu')(y)
    y = tf.keras.layers.MaxPooling2D(pool_size=2)(y)
    y = tf.keras.layers.Conv2D(kernel_size=3, filters=16, padding='same', use_bias=True, activation='relu')(y)
    y = tf.keras.layers.MaxPooling2D(pool_size=2)(y)
    y = tf.keras.layers.Conv2D(kernel_size=3, filters=16, padding='same', use_bias=True, activation='relu')(y)
    ...
    model = tf.keras.Model(x, y)
    return model'''
def get_encoder(input_shape = (INPUT_SIZE[0],INPUT_SIZE[1],1)):
  base_model = DenseNet121(weights = None,include_top=False, input_shape= (INPUT_SIZE[0],INPUT_SIZE[1],1))
  inputs = base_model.input
  last_layer = base_model.output
  x = tf.keras.layers.Flatten()(last_layer)
  output = tf.keras.layers.Dense(n_classes, activation = 'relu')(x)
  model = tf.keras.Model(inputs, output, name="return")
